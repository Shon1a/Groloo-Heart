 ...
 *   rank_streams                   DetailModal.tsx:684-685
 *   media key round trip           DetailModal.tsx:462 / :414 / :579 / :659
 *
...
/* `DetailModal.tsx:684-685` is an expression inside a React component, not a
...
const rankStreams = {
  name: 'rank_streams  (vs DetailModal.tsx:684-685)',
  twin: { at: 'Groloo-Web/src/lib/addonClient.ts:40-41 qualityRank + DetailModal.tsx:684 and :685', verdict: 'rewrite', obligation: 'rank_streams answers {ranked, summary} with indices and blocked reasons, not a filtered array — the two call sites are a UI change, not a substitution' },
  ...
  run(t, core, f) {
    ...
    assertSourceLine('src/components/DetailModal/DetailModal.tsx', 685, SHELL_SORT_SOURCE,
      'the parity corpus transcribes this sort because it is an expression inside JSX and cannot be imported.');
    assertSourceLine('src/components/DetailModal/DetailModal.tsx', 684, SHELL_FILTER_SOURCE,
...
const mediaKey = {
  name: 'media key round trip  (vs DetailModal.tsx:462 / :414 / :579)',
  twin: { at: 'Groloo-Web/src/components/DetailModal/DetailModal.tsx:462, :414, :579, :659', verdict: 'unreachable', obligation: 'MediaRef is not exported at the boundary, so there is nothing for these expressions to be replaced BY. The round trip is proven; the deletion is not available.' },
  ...
  run(t, core, f) {
    /* :462, moved from :426 (and :404, :361, :319, :312, :188 before that). Unique in the file.
     *
     * The :426 -> :462 move is +36: DetailModal grew a merge step where /api/meta and the
     * add-on meta are combined, so an anime card addressed by `kitsu:12` can borrow our
     * synopsis and our baked artwork while still being ADDRESSED by the add-on's own id.
     * The three companion numbers below moved by the same 36 and are re-pinned with it.
     *
     ...
     * (Groloo-Web 91d80fe) rather than guessed at, and they are now :414 / :579 / :659:
     *
     *   :299 → :414   the wire video id. The expression itself changed shape when the builder
     ...
     *   :433 → :579   `buildMediaFor`'s copy of the key builder. Unchanged, only moved.
     *   :458 → :659   where `playEpisode` obtains the video id. It used to interpolate the id
    ...
    assertSourceLine('src/components/DetailModal/DetailModal.tsx', 462, SHELL_KEY_SOURCE,
  ...
  fixtures: [
    ...
    },
    {
      ...
      family: {
        name: 'pin',
        run() {
          assertSourceLine('src/components/DetailModal/DetailModal.tsx', 685,
